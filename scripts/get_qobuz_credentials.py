            ...
            for secret in decoded_secrets:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, headers=headers) as response:
                        ...
                        # Or if it's specifically NOT the error code that invalid secrets yield (e.g., not 400/401/403)
                        # Adjust the condition based on Qobuz's actual API behavior for invalid secrets vs other errors
                        if response.status != 400: # Assuming 400 is the primary 'invalid secret' indicator based on original code
                            valid_secret = secret
                            break
            if not valid_secret:
                 print("Could not find a valid secret after testing candidates.") # Add a message if no secret worked
            return {"app_id": app_id, "secret": valid_secret}

    except Exception as e:
        print(f"Error fetching or processing Qobuz credentials: {e}")
        return None # Return None or raise exception on error

if __name__ == "__main__":
    credentials = asyncio.run(get_app_id_and_secrets())
    if credentials:
